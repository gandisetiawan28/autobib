      ...
      if (parsed) {
        ...
        if (tool && tool !== 'none' && ops.length > 0) {
            ...
            try {
                 ...
                 for (const op of ops) {
                     ...
                     if (act === 'replace') { await OfficeBridge.searchAndReplaceSelection([op]); executedCount.replace++; }
                         ...
                         try {
                            ...
                            const data = await res.json();
                            if (res.ok) {
                                toolMsg += `🔍 Berhasil membaca ${pathToView}. `;
                                setTimeout(() => {
                                    let fileContent = data.content;
                                    const MAX_LEN = 6000000;
                                    if (fileContent.length > MAX_LEN) {
                                        fileContent = fileContent.substring(0, MAX_LEN) + "\n\n... [SISA KODE DIPOTONG OLEH SISTEM KARENA TERLALU PANJANG. ABAIKAN JIKA ADA KODE TERPOTONG DI BARIS TERAKHIR] ...";
                                    }
                                    els.input.value = `[SYSTEM AUTO-REPORT] Tool 'view_code' berhasil dieksekusi. Berikut isi file ${pathToView}:\n\n\`\`\`javascript\n${fileContent}\n\`\`\`\n\nBerdasarkan kode di atas, berikan solusi perbaikannya pada 'message' atau 'thought'.`;
                                    if (els.btnSend) els.btnSend.click();
                                }, 1000);
                            } else {
                                toolMsg += `❌ Gagal membaca ${pathToView}: ${data.error}. `;
                            }
                         } catch (e) {
               ...
               if (toolMsg) finalHtml += '<div style="color:var(--primary);font-size:11px;padding:5px;background:var(--surface);border-radius:4px;margin-top:8px;">' + toolMsg + '</div>';
            } catch(e) {
               finalHtml += '<div style="color:var(--error);font-size:11px;padding:5px;background:var(--surface);border-radius:4px;margin-top:8px;">⚠️ Gagal mengeksekusi tool: ' + e.message + '</div>';
               
               // Auto-Feedback Loop for Tool Execution Errors
               setTimeout(() => {
                   if (els.input) {
                       els.input.value = `[SYSTEM AUTO-REPORT] Gagal mengeksekusi tool. Error: ${e.message}\nDirektori project root: d:\\1. MY CODE\\AUTOBIB\nSilakan gunakan tool 'view_code' dengan parameter "path" menuju 'frontend/assets/js/office-bridge.js' (atau file relevan lainnya) untuk menganalisis mengapa perintah tersebut gagal dan berikan solusinya pada 'message' atau 'thought'.`;
                       if (els.btnSend) els.btnSend.click();
                   }
               }, 2000);
            }

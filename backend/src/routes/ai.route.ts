...
function buildPrompt(mode: string, abstracts: string[], customPrompt: string, language: string): { system: string, user: string, full: string } {
...
router.post('/generate', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!abstracts.length) return next(createError('abstracts array is required', 400));
    ...
    const settings = db
      ...
      .get(req.userId) as { active_provider: Provider; output_language: string; max_retry: number; retry_delay_ms: number; local_bridge_url?: string } | undefined;

    const activeProvider: Provider = provider ?? settings?.active_provider ?? 'gemini';
    ...
    try {
      if (activeProvider.startsWith('local_')) {
        ...
        try {
          ...
          if (e.code === 'ECONNREFUSED') sendEvent('error', { message: 'Local Bridge tidak berjalan (port 3000)' });
          else sendEvent('error', { message: 'Local Bridge error: ' + e.message });
        }
      } else {
        await withRetry(
          {
          ...
          async (apiKey, keyId) => {
            ...
            // Save to history
            db.prepare(
              `INSERT INTO generate_history (id, user_id, provider, key_id, mode) VALUES (?, ?, ?, ?, ?)`
            ).run(uuidv4(), req.userId, activeProvider, keyId, mode);
          }
        );
        sendEvent('done', { success: true });
      }
      ...
      sendEvent('error', { message: msg, code: (err as { code?: string }).code });
    }
...
router.post('/fix-metadata', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { documents } = req.body;
    if (!documents || !Array.isArray(documents)) return next(createError('documents array is required', 400));
...
${JSON.stringify(documents, null, 2)}`;
    ...
    if (provider.startsWith('local_')) {
      ...
      try {
        ...
        if (parsed.fixed_documents) parsed = parsed.fixed_documents;
        if (parsed.documents) parsed = parsed.documents;

...
WARNING: For anchor fields, you MUST copy EXACT verbatim text from the document. The system uses exact string matching.
- Use "find-single" for a SINGLE text anchor.
- Use "find-start" + "find-end" for a TEXT RANGE (bookend search).
- Legacy "find" is still supported for backward compatibility, but new tasks SHOULD prefer "find-single"/"find-start"/"find-end".
...
defaultRegistry.registerTool({
  ...
  examples: [
    'operations: [ {"find-single": "exact wrong word in text", "replace": "correct word"} ]',
    'CRITICAL CONTEXT RULE: When fixing grammar or typos, you MUST preserve the original semantic meaning and context.',
    'To replace ALL occurrences of the same typo in the document: operations: [ {"find-single": "typo", "replace": "correct", "replace_all": true} ]',
    'To rewrite a whole paragraph: operations: [ {"find-single": "first 5-10 words of the paragraph", "replace": "teks baru dengan kata asing seperti *machine learning* dicetak miring!", "target_type": "paragraph"} ]',
    'To apply a specific Word style (e.g. from [AVAILABLE STYLES]), add the "style" property: operations: [ {"find-single": "...", "replace": "...", "style": "Heading 2,SUB-BAB 1"} ]'
  ]
});
...
defaultRegistry.registerTool({
  ...
  examples: [
    'operations: [ {"find-single": "Copy the ENTIRE sentence here exactly as it appears", "comment": "Consider rewriting this."} ]',
    'To comment on a whole paragraph: operations: [ {"find-single": "first 10 words of the paragraph", "comment": "This paragraph is weak.", "target_type": "paragraph"} ]'
  ]
});
...
defaultRegistry.registerTool({
  ...
  examples: [
    'operations: [ {"find-single": "Copy the ENTIRE sentence to highlight here", "color": "Yellow"} ]',
    'To highlight a whole paragraph: operations: [ {"find-single": "first 5-10 words", "color": "Yellow", "target_type": "paragraph"} ]'
  ]
});
...
defaultRegistry.registerTool({
  ...
  examples: [
    'To format an entire word: operations: [ {"find-single": "exact word to format", "apply": "bold"} ]',
    'To format ONLY a specific character/part within a word (e.g. the 2 in O2): operations: [ {"find-single": "O2", "target": "2", "apply": "subscript"} ]'
  ]
});
...
defaultRegistry.registerTool({
  ...
  examples: [
    'operations: [ {"find-single": "exact sentence or word to delete"} ]',
    'CRITICAL WARNING: To delete an entire line or paragraph, you MUST add `"target_type": "paragraph"`. If you forget this, empty blank lines (paragraph breaks) will be left behind in the document! Example: operations: [ {"find-single": "first 5-10 words of the paragraph", "target_type": "paragraph"} ]'
  ]
});

...
router.post('/parse', async (req, res, next) => {
    try {
        ...
        const getLocalResponse = async (systemInstruction, promptText, bridgeProvider) => {
            try {
                const response = await axios_1.default.post(`${bridgeUrl}/api/${bridgeProvider}/chat/completions`, {
                    messages: [
                ...
                if (e.code === 'ECONNREFUSED')
                    throw new Error(`Local Bridge tidak berjalan. Jalankan server.js di folder Super Skripsi terlebih dahulu (${bridgeUrl}).`);
                if (e.code === 'ECONNABORTED' || e.message?.includes('timeout'))
                    throw new Error(`Local Bridge timeout. Pastikan ekstensi Super Skripsi aktif di browser dan tab ${bridgeProvider} terbuka.`);
...
4. Automatically convert the word "dan" (or "and") to the ampersand symbol "&" in any prefix or suffix you generate! (e.g. if the text is "Mathis dan Jackson (2006) dalam", your prefix MUST be "Mathis & Jackson (2006) dalam ").`;
            ...
            try {
                ...
                results = texts.map((t, idx) => {
                    ...
                    // Clean items
                    items = items.filter((i) => i && typeof i === 'object' && Object.keys(i).length > 0 && !Array.isArray(i));
                    if (items.length === 0)
                        items = [{ author: null, year: null }];
...
router.post('/extract-full', async (req, res, next) => {
    try {
        const { text } = req.body;
        if (!text)
            return next({ statusCode: 400, message: 'Text required' });
...
${text.substring(0, 150000)}`;
        const db = await Promise.resolve().then(() => __importStar(require('../utils/database'))).then(m => m.getDb());
        const settings = db.prepare('SELECT active_provider, local_bridge_url FROM user_settings WHERE user_id = ?').get(req.userId);
        const provider = settings?.active_provider ?? 'gemini';
        const bridgeUrl = settings?.local_bridge_url || 'http://127.0.0.1:3000';
        ...
        if (provider.startsWith('local_')) {
            ...
            try {
                ...
                if (e.code === 'ECONNREFUSED')
                    throw new Error(`Local Bridge tidak berjalan. Jalankan server.js di Super Skripsi (${bridgeUrl}).`);
                if (e.code === 'ECONNABORTED' || e.message?.includes('timeout'))
                    throw new Error(`Local Bridge timeout. Pastikan ekstensi aktif di browser dan tab ${bridgeProvider} terbuka.`);
                throw new Error(`Local Bridge error: ${e.message}`);
            }
        }
        else {
            await (0, retry_handler_1.withRetry)({ userId: req.userId, provider }, async (apiKey) => {
                const { GoogleGenerativeAI } = await Promise.resolve().then(() => __importStar(require('@google/generative-ai')));
                const c = new GoogleGenerativeAI(apiKey);
                const r = await c.getGenerativeModel({ model: 'gemini-2.5-flash' }).generateContent(prompt);
                rawJson = r.response.text().replace(/```json\n?/g, '').replace(/```/g, '').trim();
...
router.post('/resolve', async (req, res, next) => {
    try {
        ...
        const db = (0, database_1.getDb)();
        let mendeleyToken = null;
        try {
            const { getMendeleyToken } = await Promise.resolve().then(() => __importStar(require('./mendeley.route')));
            mendeleyToken = await getMendeleyToken(req.userId);
        }
        catch { }
        const results = await Promise.all(parsed.map(async (group) => {
            const items = group.items || [];
            const resolvedItems = await Promise.all(items.map(async (item) => {
                const hash = (0, crypto_1.sha256)(JSON.stringify(item));
                const cached = db.prepare('SELECT * FROM citation_cache WHERE raw_text_hash = ?').get(hash);
                ...
                if (mendeleyToken) {
                    try {
                        ...
                        if (q) {
                            ...
                            const mRes = await axios_1.default.get('https://api.mendeley.com/documents', {
                            ...
                            let mDoc = docs.find((doc) => {
                                const docYear = parseInt(String(doc.year));
                                if (item.title && doc.title && doc.title.toLowerCase().includes(item.title.toLowerCase()))
                                    return true;
                                if (targetYear && docYear === targetYear && targetAuthor) {
                                    const docAuthors = (doc.authors || []).map((a) => (a.last_name || '').toLowerCase());
                                    if (docAuthors.some((a) => a.includes(targetAuthor)))
                                        return true;
                                }
                                return false;
                            });
                            if (mDoc) {
                                const csl = {
                                    title: mDoc.title,
                                    author: (mDoc.authors || []).map((a) => ({ family: a.last_name, given: a.first_name })),
                                ...
                                db.prepare(`INSERT OR REPLACE INTO citation_cache (id, raw_text, raw_text_hash, csl_json, mendeley_uuid, resolve_source, resolve_status) VALUES (?, ?, ?, ?, ?, 'mendeley_search', 'found')`).run(mDoc.id, group.raw_text ?? item.title ?? '', hash, JSON.stringify(csl), mDoc.id);
                ...
                if (item.doi || (item.title && item.title.length > 5)) {
                    try {
                        ...
                        const crossrefRes = await axios_1.default.get(query, { headers, timeout: 8000 });
                        let work = item.doi ? crossrefRes.data.message : null;
                        if (!work && crossrefRes.data.message.items) {
                            work = crossrefRes.data.message.items.find((w) => {
                                if (item.title)
                                    return true;
                                return false;
                        ...
                        if (work) {
                            const csl = {
                                ...
                                author: (work.author || []).map((a) => ({ family: a.family, given: a.given })),
                            ...
                            db.prepare(`INSERT OR REPLACE INTO citation_cache (id, raw_text, raw_text_hash, csl_json, resolve_source, resolve_status) VALUES (?, ?, ?, ?, 'crossref', 'found')`).run((0, uuid_1.v4)(), group.raw_text ?? item.title ?? '', hash, JSON.stringify(csl));
                ...
                if (item.title) {
                    try {
                        const ssRes = await axios_1.default.get(`https://api.semanticscholar.org/graph/v1/paper/search?query=${encodeURIComponent(item.title)}&fields=title,authors,year,externalIds,venue&limit=1`, { timeout: 8000 });
                        ...
                        if (paper) {
                            ...
                            db.prepare(`INSERT OR REPLACE INTO citation_cache (id, raw_text, raw_text_hash, csl_json, resolve_source, resolve_status) VALUES (?, ?, ?, ?, 'semantic_scholar', 'found')`).run((0, uuid_1.v4)(), group.raw_text ?? item.title ?? '', hash, JSON.stringify(csl));
                ...
                db.prepare(`INSERT OR REPLACE INTO citation_cache (id, raw_text, raw_text_hash, csl_json, resolve_source, resolve_status) VALUES (?, ?, ?, ?, 'ai', 'partial')`).run((0, uuid_1.v4)(), group.raw_text ?? item.title ?? '', hash, JSON.stringify(csl));
...
router.post('/build-field', (_req, res, next) => {
    try {
        const { items, formatted_citation } = _req.body;
        if (!items || !items.length)
            return next({ statusCode: 400, message: 'items required' });
        ...
        const citationItems = items.map((item) => {
            const mUuid = item.mendeley_uuid ?? (0, uuid_1.v4)();
            return {
                id: mUuid,
                itemData: {
                    ...item.csl_json,
        ...
        const citationData = {
            citationID: `MENDELEY_CITATION_${(0, uuid_1.v4)()}`,
...
//# sourceMappingURL=smart-citation.route.js.map